...
async function analyzeDiets(barcode) {
  ...
  try {
    const product = window.currentProduct || currentProductCache[barcode];
    const result = await aiService.analyzeDietCompatibility(
      product,
      selectedDiets
    );
...
function formatAIResponse(content) {
  ...
  try {
          ...
          html += `
            <div class="mood-card">
              <div class="mood-card-header">
                ...
                <span class="energy-badge ${energyClass}">${energyIcon} ${
            item.energy_level || "Sustain"
          }</span>
              </div>
              ...
              ${
                ...
                <div class="tags-container">
                  ${item.key_nutrients
                    .split(",")
                    .map((n) => `<span class="nutrient-tag">${n.trim()}</span>`)
                    .join("")}
                </div>
              </div>`
                  : ""
      ...
      parsed.forEach((item, index) => {
        ...
        html += `
          <div class="alternative-card">
            ...
            ${
              item.why_it_s_healthier
                ? `<p class="alternative-reason"><strong>Why it's healthier:</strong> ${item.why_it_s_healthier}</p>`
                : ""
            }
            ${
            ...
            ${
              item.estimated_price_range
                ? `<p class="price-range">💰 ${item.estimated_price_range}</p>`
                : ""
            }
          </div>
        `;
    ...
    if (
      parsed.eco_score ||
      parsed["carbon_footprint"] ||
      parsed["overall_eco_score"]
    ) {
      ...
      if (parsed.carbon_footprint) {
        html += `<div class="eco-item">
          ...
          <div class="eco-info">
            ...
            <span class="eco-value ${parsed.carbon_footprint.toLowerCase()}">${
          parsed.carbon_footprint
        }</span>
          </div>
        </div>`;
      ...
      if (parsed.water_usage) {
        html += `<div class="eco-item">
          ...
          <div class="eco-info">
            ...
            <span class="eco-value ${parsed.water_usage.toLowerCase()}">${
          parsed.water_usage
        }</span>
          </div>
        </div>`;
      ...
      if (parsed.transportation_impact) {
        html += `<div class="eco-item">
          ...
          <div class="eco-info">
            ...
            <span class="eco-value ${parsed.transportation_impact.toLowerCase()}">${
          parsed.transportation_impact
        }</span>
          </div>
        </div>`;
      }
        ...
        html += `<div class="eco-item">
          ...
          <div class="eco-info">
            ...
            <div class="mini-bar-container">
              <div class="mini-bar-fill" style="width: ${
                parsed.packaging_sustainability_score * 10
              }%"></div>
              <span class="mini-bar-label">${
                parsed.packaging_sustainability_score
              }/10</span>
            </div>
          </div>
        </div>`;
          ...
          <div class="stat-item not-compatible">
            ...
            <div class="stat-info">
              <span class="stat-number">${
                totalDiets - compatibleCount - maybeCount
              }</span>
    ...
    return (
      '<pre class="json-result">' + JSON.stringify(parsed, null, 2) + "</pre>"
    );
  } catch (e) {
    ...
    formatted = formatted.replace(
      /^\*\s+(.*?):/gm,
      '<p class="list-item"><strong>$1:</strong>'
    );
    ...
    formatted = formatted
      ...
      .map((para) => {
        if (
          para.trim() &&
          !para.includes("<h4>") &&
          !para.includes('<p class="list-item">')
        ) {
